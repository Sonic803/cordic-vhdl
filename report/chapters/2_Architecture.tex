...
Focusing on the module, \( \rho \) is unsigned so that outputs with \( \rho \geq 128 \) can be represented. For the intermediate calculation the representation has 2 more bits for the integer part, this ensures that there isn't an overflow on x during the calculations.
